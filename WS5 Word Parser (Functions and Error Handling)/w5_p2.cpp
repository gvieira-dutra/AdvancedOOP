...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		std::ifstream bkFile(argv[1]);
		
		std::string thisLine{};
		int counter{};

		do {
			std::getline(bkFile, thisLine);
	...
	auto moneyConversion = [usdToCadRate, gbpToCadRate](Book& src) {
		if (!std::strcmp(src.country().c_str(), "US"))
		{
			src.price() = src.price() * usdToCadRate;
		}
		else if (!std::strcmp(src.country().c_str(), "UK") && src.year() >= 1990 && src.year() <= 1999)
		{
	...
	if (argc > 2) {
		...
		std::ifstream mvFile(argv[2]);

		std::string thisLine;
		int counter{};

		do {
			std::getline(mvFile, thisLine);

			if (mvFile)
			{
				if (thisLine[0] != '#')
				{
					movies[counter] = thisLine;
					counter++;
				}
			}
		} while (counter < 5);
